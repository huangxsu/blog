...
让我们看个例子，求下图$v\_{0}$到其余各定点的最短路径，来理解 Dijkstra 算法：

{% asset_img Dijkstra.jpg %}

首先，$v\_{0}$是起点，列出$v\_{0}$到其余各个顶点的距离：

| S        | $v\_{1}$ | $v\_{2}$ | $v\_{3}$ | $v\_{4}$ | $v\_{5}$ |
| -------- | -------- | -------- | -------- | -------- | -------- |
| $v\_{0}$ | $\infty$ | 10       | $\infty$ | 30       | 100      |

接下来从上表中选取距离最小的点$v\_{2}$，计算$v\_{2}$到其各个邻接点的距离，若计算的结果比上一步的结果小则替换，这里到$v\_{3}$的值被替换为 10+50 =60，下面将重复这个过程：
...
最后剩下$v\_{5}$，而$v\_{5}$没有出边，我们的算法到这里结束。从$v\_{0}$到每个顶点的最短路径的结果是上表中最后一行。
...
弗洛伊德算法是解决任意两点间的最短路径的一种算法，可以正确处理有向图或有向图或负权（但不可存在负权回路)的最短路径问题，Floyd-Warshall 算法的原理是动态规划。
...
因为此时我们还不知道有点 1 直接到点 3 的距离是最小的，还是由点 1 到点 2 在到点 3 的距离是最小的，所以我们将通过有中间点和无中间点两个矩阵比较最短路径。
...
接下来填充矩阵$A^{2}$，先看$A^{2}\left [ 1,3 \right ]$处应该如何填入，原来$A^{1}\left [ 1,3 \right ] = \infty$，假设以点 2 为中间点：$A^{2}\left [ 1,2 \right ] + A^{2}\left [ 2,3 \right ] = 5$，所以$A^{2}\left [ 1,3 \right ]$填入较小的值 5。填入矩阵$A^{2}$的值不在以$A^{0}$而以$A^{1}$为基础进行比较，余空格以此类推，最终我们得到以点 2 为中间点是的最短路径矩阵：
