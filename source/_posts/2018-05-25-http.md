...
持久连接是建立 1 次 TCP 连接后进行多次请求和响应。在 HTTP/1.1 中，所有的连接默认都是持久连接。

{% asset_img keep-alive.png 500 keep-alive%}

持久连接带来的另外一个好处就是并发请求，以前发生请求后需等待并接收响应才能发下一个。持久连接使得多数请求以管线化的方式发送。

上面提到了TCP三次握手建立连接，下面来说一说TCP四次挥手释放连接：

{% asset_img tcp-four-way-handshake.png  四次挥手 %}

数据传输结束后，通信双方都可以释放连接。A的应用进行先向其TCP发出连接释放报文段，并停止再发送数据，主动关闭TCP连接，报文首部FIN标志，进入FIN-WAIT-1状态，B收到报文后发出确认ACK，进入CLOSE-WAIT状态，此时TCP连接处于半关闭状态，即A已经没有数据要发送了，但B若发生数据，A仍要接收。A收到B的确认后进入FIN-WAIT-2状态，若B已经没有要发送的数据了，就通知TCP释放连接，B发送释放报文FIN，A收到B的释放报文后，必须发出确认ACK报文。现在TCP连接还没有释放，必须经过**时间等待计时器**设置的时间2MSL后，A才进入CLOSED状态。MSL叫做**最长报文段寿命**，RFC793建议设为2分钟。从A进入TIME-WAIT状态后，要经过4分钟才能进入CLOSED状态，才能开始建立下一个新的连接。
